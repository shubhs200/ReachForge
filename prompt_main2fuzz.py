...
def build_main2fuzz_prompt(root: Path, out_dir: Path, *, include_vulns: bool = False, include_poller: bool = True) -> Optional[Path]:
    ...
    lines.append("- Be compatible with AFL++-style persistent mode: structure the main fuzz logic so it can run inside a guarded loop (for example, while (__AFL_LOOP(N))) without relying on fork-per-input. Within that loop, read a fresh input, process it once, and then reset all per-iteration state before the next iteration.")
    ...
    lines.append("- If the codebase exposes multiple decoders or format handlers, select at most one decoder or logical code path per input based on cheap, deterministic checks (for example, magic bytes or a type enum) instead of running all decoders on every buffer or falling back through many codecs when the type is unknown.")
