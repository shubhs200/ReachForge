...
def build_main2fuzz_prompt(root: Path, out_dir: Path, *, include_vulns: bool = False, include_poller: bool = True) -> Optional[Path]:
    ...
    lines.append("- In that case, reuse the same input buffer in multiple ways: (1) treat it as a high-level script/program that is compiled/loaded/executed; (2) treat it as a raw blob passed to core data-creation APIs (for example, functions that create strings, tables, arrays, or similar containers); and (3) drive APIs that operate on containers with index/range/offset parameters.")
    lines.append("- For container+index/range APIs, build a container (array/list/table/etc.) from a prefix of the fuzz buffer and derive indices, offsets, or bounds from later bytes (for example, interpret 4 or 8 tail bytes as signed/unsigned integers, clamp into valid ranges, and occasionally swap them so start <= end while still exploring start > end).")
