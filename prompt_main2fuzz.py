...
def build_afg_prompt(root: Path, out_dir: Path, afg_path: Path) -> Optional[Path]:
    ...
    lines.append("- The harness must be a single-shot stdin/file driver (AFL '@@' compatible): if argc > 1 and argv[1] != '-', read input bytes from that file; otherwise read from stdin.")
    lines.append("- Cap input to MAX_INPUT = 64 * 1024. If len == 0 or len > MAX_INPUT: do nothing and exit 0.")
    lines.append("- Initialize Lua with luaL_newstate(); then call luaL_openlibs(L) so that global 'unpack' is registered.")
    lines.append("- The harness MUST follow this 3-stage flow in this order (all driven by the fuzz buffer):")
    lines.append("  1) luaL_loadbuffer(L, data, len, \"fuzz_chunk\"); if and only if it returns 0, lua_pcall(L, 0, LUA_MULTRET, 0). Always pop the error object on failure (both load and pcall failures).")
    lines.append("     IMPORTANT: after stage (1), ALWAYS reset the Lua stack with lua_settop(L, 0) so no return values/errors leak into later stages.")
    lines.append("  2) lua_pushlstring(L, (const char*)data, len); then lua_pop(L, 1).")
    lines.append("  3) Call a helper that drives the GLOBAL function unpack via the C API (lua_getglobal + lua_pcall), using a fuzz-built table + two fuzz-derived indices.")
    lines.append("- Stage 3 MUST call global unpack (lua_getglobal(L, \"unpack\")) and REQUIRE it to be a function (no fallback).")
    lines.append("- FORBIDDEN in this target: string.unpack, table.unpack, lua_rawseti, and executing Lua code strings to reach unpack.")
    lines.append("  Only call global unpack via lua_getglobal + lua_pcall, and build the table using lua_createtable + lua_settable with explicit key/value pushes.")
    lines.append("- Stage 3 table construction rules: build a dense table from a small prefix of the input (for example: take nbytes = min(len, 1024) and nelems = min(nbytes, 256); for i in 0..nelems-1 set tbl[i+1] = data[i] as an integer).")
    lines.append("- Derive istart/iend as signed little-endian int32 from the LAST 8 bytes of input (default range: 1..nelems). If istart > iend, swap them. Do NOT clamp indices into a narrow range.")
    lines.append("- After calling unpack via lua_pcall in stage (3), ALWAYS reset the Lua stack with lua_settop(L, 0) whether the call succeeded or failed (after popping the error object on failure).")
    lines.append("- It is OK (and recommended) to implement small static helper functions such as process_one_input() and fuzz_unpack() to keep the code clear.")
    lines.append("")
