...
def build_afg_prompt(root: Path, out_dir: Path, afg_path: Path) -> Optional[Path]:
    ...
    lines.append("- In particular, when vulnerabilities.json lists both luaS_newlstr (for example, in src/lstring.c) and luaB_unpack (for example, in src/lbaselib.c) as vulnerable and we are building a harness for lua in redis you MUST follow these steps. REQUIRED Lua initialization and includes: use luaL_newstate() (NOT lua_open()), and call luaL_openlibs(L) before accessing any globals. Include <lua.h>, <lauxlib.h>, and <lualib.h> (NOT redis internal headers like script_lua.h). Then structure the harness in three stages: first call luaL_loadbuffer(L, buf, len, 'fuzz_chunk') followed by lua_pcall(...) on the entire fuzz buffer, then push the entire buffer once with lua_pushlstring(L, (const char*)buf, len) and pop it, and finally build a dense table from a prefix of the buffer (for example, indices 1..N with N <= 256, storing integer values derived from bytes) and call the global 'unpack' function via the C API on that table with two fuzz-derived index arguments.")
    ...
    lines.append("")
    lines.append("**CRITICAL CORRECTION – COMMON MISTAKES IN PREVIOUS GENERATIONS (you MUST avoid these):**")
    lines.append("- COMMON CRITICAL ERROR: Do NOT call lua_pcall immediately after luaL_loadbuffer without checking the result. On syntax errors (most inputs), loadbuffer pushes an error string, not a function. You MUST check the return code: only pcall if load succeeded, and always pop the error object if load failed — otherwise the stack is corrupted for later stages.")
    lines.append("- Do NOT use redis internal Lua glue headers (e.g., script_lua.h) or lua_open(); this harness is about exercising the vulnerable Lua runtime code via the PUBLIC Lua C API.")
    lines.append("- Do NOT call table.unpack or string.unpack via Lua code strings; call global unpack via lua_getglobal + lua_pcall so you reach luaB_unpack directly.")
    lines.append("")
    lines.append("**REQUIRED CORRECT BEHAVIOR FOR THIS LUA TARGET (self-contained; do NOT assume any reference harness exists):**")
    lines.append("- This section applies ONLY when the AFG/vulnerabilities include BOTH luaB_unpack and luaS_newlstr (Lua target).")
    lines.append("- Includes must be ONLY via DriverSpec 'includes' (do NOT write any '#include' lines inside driver_source).")
    lines.append("- The includes list MUST use the public Lua headers exactly as: <lua.h>, <lauxlib.h>, <lualib.h> (angled, not quoted).")
    lines.append("- Do NOT include redis internal Lua glue headers (e.g., script_lua.h) or include Lua via project-relative paths like \"deps/lua/src/lua.h\" or any vcpkg/buildtrees paths.")
    lines.append("- Input handling MUST be AFL '@@' compatible AND match this pattern:")
    lines.append("  - If argc > 1 and argv[1] != '-', read from that file path using fseek/ftell to size the allocation (cap to MAX_INPUT) and fread the file.")
    lines.append("  - Else, read from stdin using a realloc-growing buffer (start 4096, double, cap at MAX_INPUT).")
    lines.append("- Cap input to: #define MAX_INPUT (64 * 1024). If len == 0 or len > MAX_INPUT: do nothing and exit 0.")
    lines.append("- Use uint8_t* for the input buffer (not unsigned char*), and keep the code single-file C.")
    lines.append("- The driver_source MUST follow this exact STRUCTURE (names and stages matter; do not invent additional stage_* helpers):")
    lines.append("  1) static void fuzz_unpack(lua_State *L, const uint8_t *data, size_t len) { ... }")
    lines.append("  2) static void process_one_input(const uint8_t *data, size_t len) { ... }")
    lines.append("  3) int main(int argc, char **argv) { ... }")
    lines.append("- process_one_input() MUST create a fresh lua_State per input (luaL_newstate + luaL_openlibs) and then run exactly these 3 stages in this order:")
    lines.append("  1) luaL_loadbuffer(L, (const char*)data, len, \"fuzz_chunk\"); if and only if it returns 0, lua_pcall(L, 0, LUA_MULTRET, 0). On ANY error, pop exactly one error object.")
    lines.append("  2) lua_pushlstring(L, (const char*)data, len); lua_pop(L, 1);")
    lines.append("  3) fuzz_unpack(L, data, len);")
    lines.append("  Then lua_close(L).")
    lines.append("- fuzz_unpack() MUST call the GLOBAL function unpack via C API (lua_getglobal + lua_pcall).")
    lines.append("- FORBIDDEN in this target: string.unpack, table.unpack, lua_rawseti, and executing Lua code strings to reach unpack. Only global unpack.")
    lines.append("- fuzz_unpack() table construction MUST use lua_createtable + lua_settable with explicit key/value pushes (key=i+1, value=(lua_Integer)data[i]).")
    lines.append("- fuzz_unpack() MUST implement the same bounds shaping:")
    lines.append("  - nbytes = MIN(len, 1024); nelems = MIN(nbytes, 256)")
    lines.append("  - default istart=1, iend=nelems")
    lines.append("  - if len>=8, derive istart/iend from the LAST 8 bytes as two little-endian int32 values; if istart > iend, swap them")
    lines.append("  - call unpack(tbl, istart, iend) via lua_pcall, ignore errors (pop error), and finally lua_settop(L, 0)")
    lines.append("- Provide #ifndef MIN / #define MIN(a,b) ... guard in driver_source, and use MIN exactly as described above.")
    lines.append("")
