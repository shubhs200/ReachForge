...
def build_afg_prompt(root: Path, out_dir: Path, afg_path: Path) -> Optional[Path]:
    ...
    lines.append("")
    lines.append("**REQUIRED CORRECT BEHAVIOR FOR THIS LUA TARGET (MATCH redis/redis_fuzz.c STYLE):**")
    lines.append("- Includes must be ONLY via DriverSpec 'includes' (do NOT write any '#include' lines inside driver_source).")
    lines.append("- Use Lua public headers: <lua.h>, <lauxlib.h>, <lualib.h> (do NOT include redis internal headers like script_lua.h for this harness).")
    lines.append("- Cap input size to MAX_INPUT = 64*1024.")
    lines.append("- Input handling must match redis_fuzz.c: support argv[1] file input (AFL @@) if provided and not equal to '-', else read from stdin. Dynamic allocation (malloc/realloc) is allowed as long as it is bounded by MAX_INPUT.")
    lines.append("- Initialize Lua with luaL_newstate(); then call luaL_openlibs(L) so that global 'unpack' is registered.")
    lines.append("- Use the same 3-stage structure as redis_fuzz.c:")
    lines.append("  1) luaL_loadbuffer on the entire input and if it succeeds, lua_pcall it; always pop the error object on failure.")
    lines.append("  2) lua_pushlstring(L, data, len) then pop it.")
    lines.append("  3) Call a helper that drives global unpack with a fuzz-built table + fuzz-derived indices.")
    lines.append("- Stage 3 must : get global 'unpack' and require it to be a function (no fallback). Build the table with lua_createtable + lua_settable (push key=i+1 then value=data[i]).")
    lines.append("- Derive indices i/j from the last 8 bytes as two little-endian int32 values; set default range 1..nelems. If istart > iend, swap them (always) like redis_fuzz.c.")
    lines.append("- It is OK to structure the driver with small static helper functions like redis_fuzz.c (e.g., process_one_input() and fuzz_unpack()).")
    lines.append("")
