...
def build_afg_prompt(root: Path, out_dir: Path, afg_path: Path) -> Optional[Path]:
    ...
    lines.append("- Do NOT use dynamic allocation (malloc/realloc) or file arguments – this makes the driver slow and incompatible with many fuzzers.")
    lines.append("- Do NOT add __AFL_LOOP or persistent-mode loops – this target uses simple single-shot stdin harnesses.")
    lines.append("- Do NOT always swap the unpack indices or force start ≤ end every time – you MUST only occasionally swap (e.g., ~50% probability) so that inverted, negative, and extreme index cases remain reachable.")
    ...
    lines.append("- Exactly three stages, always in this order:")
    lines.append("  1. Load and execute the entire input buffer as a Lua chunk (ignore all errors).")
    lines.append("  2. Push the entire raw buffer as one Lua string and immediately pop it.")
    lines.append("  3. Build a small dense table from only a bounded prefix of the input (max ~256 elements), using raw table insertion, then call the global 'unpack' via the C API with two fuzz-controlled signed 32-bit indices.")
    lines.append("- Ensure the global 'unpack' exists (fall back to table.unpack if needed).")
    lines.append("- Derive the two indices as signed 32-bit little-endian values from two different positions at the tail of the input.")
    lines.append("- Occasionally (not always) swap the indices so that start ≤ end most of the time but inverted cases are still explored.")
    lines.append("- No extra sources, no lifted functions, no unnecessary includes or comments.")
    lines.append("")
