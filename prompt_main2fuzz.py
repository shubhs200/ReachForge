...
def build_afg_prompt(root: Path, out_dir: Path, afg_path: Path) -> Optional[Path]:
    ...
    lines.append("- After exercising such a high-level load/parse step, also call at least one lower-level or more focused API directly with fuzz-derived arguments (for example, slices of the same buffer) so that both the parser/compiler and the primitive are exercised in a single execution.")
